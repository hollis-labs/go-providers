...
func TestClaudeAdapter_PTYSpawn_Smoke(t *testing.T) {
	...
	// Single goroutine owns cmd.Wait — exec.Cmd.Wait must only be called
	// once. The buffered+closed channel lets both the main test path and
	// t.Cleanup observe the result without racing on Wait.
